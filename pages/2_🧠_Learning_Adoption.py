...
active_counts.columns = [breakdown_variable, 'active_count']
...
# Group active_learners by the selected breakdown variable and get unique nik_counts
active_nik_by_unit = active_learners.groupby(breakdown_variable)['nik'].nunique().reset_index()
active_nik_by_unit.columns = [breakdown_variable, 'active_nik_count']

# Merge to get passive counts by the selected breakdown variable
passive_counts = pd.merge(all_nik_in_sap, active_nik_by_unit, on=breakdown_variable, how='left').fillna(0)
passive_counts['passive_count'] = passive_counts['all_nik_count'] - passive_counts['active_nik_count']
passive_counts.drop(columns=['all_nik_count', 'active_nik_count'])
...
final_counts['Active Learners'] = final_counts['active_count'] / final_counts['all_nik_count'] * 100
final_counts['Passive Learners'] = final_counts['passive_count'] / final_counts['all_nik_count'] * 100

# Sort units based on the total learners
final_counts = final_counts.sort_values(by=breakdown_variable, ascending=False)

# Check final_counts result
print(final_counts)
...
total_active_learners = final_counts['active_count'].sum()
total_learners = final_counts['all_nik_count'].sum()
...
base = alt.Chart(final_counts).transform_fold(
    fold=['Active Learners', 'Passive Learners'], # Note the order
    as_=['Learner Type', 'percent']
).mark_bar().encode(
    x=alt.X(f'{breakdown_variable}:N', sort=final_counts[breakdown_variable].tolist(), axis=alt.Axis(title=breakdown_variable.capitalize())),
    y=alt.Y('percent:Q', axis=alt.Axis(title='Learning Adoption (%)'), scale=alt.Scale(domain=[0, 100])),    # Removing the title
    ...
    tooltip=[
        ...
        alt.Tooltip('active_count:Q', title='Total Active Learners'),
        alt.Tooltip('passive_count:Q', title='Total Passive Learners'),        
